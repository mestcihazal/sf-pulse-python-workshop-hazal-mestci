...
def _settled[T](result: Any, label: str, fallback: T) -> T:
...
async def daily_refresh() -> dict[str, int]:
    ...
    logger.info("[workflow] fetching all sources...")
    (
    ...
    logger.info("[workflow] candidates: %d restaurants, %d events", len(restaurants), len(events))
